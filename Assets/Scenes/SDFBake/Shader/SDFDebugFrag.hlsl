...
PackedVaryingsType SDFDebugVert(AttributesMesh inputMesh, AttributesPass inputPass)
{
...
PackedVaryingsToPS SDFDebugVertTesselation(VaryingsToDS input)
{
    VaryingsToPS output;
...
PackedVaryingsType SDFDebugVert(AttributesMesh inputMesh)
{
    VaryingsType varyingsType;
    varyingsType.vmesh = VertMesh(inputMesh);
...
void SDFDebugFrag(PackedVaryingsToPS packedInput,
...
)
{
    ...
    // outColor = outResult;
    float3 worldSpacePos = GetAbsolutePositionWS(input.positionRWS);

    float d = GetSignedDistance(worldSpacePos) * .5;

    outColor = saturate(float4(d.xxx, 1));
    // outColor = saturate(float4(worldSpacePos, 1));
